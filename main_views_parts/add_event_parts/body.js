...
const AddEventBody = (props) => {
  ...
  const createEvent = () => {
    if (
      !props.reducer.eventTime ||
      !props.reducer.eventDate ||
      mode === "Cancel" ||
      !name ||
      !description ||
      type === "Cancel"
      ...
      alert(
        `Success! You event is scheduled for ${moment(
          props.reducer.eventDate
        ).format("ll")} ${moment(props.reducer.eventTime).format("LT")}.`
      );
    }
  };
  ...
  return (
    <ScrollView>
      ...
      >
        Here you can create games, training sessions, touranments, and other
        events. Choose your type, select time, and write a short description.
      </Text>
      ...
      <ListItem>
        <Left>
          <ModePicker SendModeFromPicker={ReceiveModeFromPicker} />
        </Left>
        <Text>Mode</Text>
      </ListItem>
      <ListItem>
        <Left>
          <Button onPress={props.toggle_eventdate_modal}>
            <Text>Date</Text>
          </Button>
        </Left>
        <Text>Date</Text>
      ...
      <ListItem>
        <Left>
          <Button onPress={props.toggle_eventtime_modal}>
            <Text>Time</Text>
          </Button>
        </Left>
        <Text>Time</Text>
...
const mapDispachToProps = (dispatch) => {
  return {
    toggle_eventtime_modal: () =>
      dispatch({ type: "TOGGLE_EVENTTIME_MODAL", value: true }),
    toggle_eventdate_modal: () =>
      dispatch({ type: "TOGGLE_EVENTDATE_MODAL", value: true }),
    //storePreCheck: () => dispatch({ type: "STORE_PRECHECK", value: true})
  };
};

export default connect(mapStateToProps, mapDispachToProps)(AddEventBody);
