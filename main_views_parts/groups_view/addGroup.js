...
import {StyleSheet,Modal, TouchableOpacity,ScrollView,ActivityIndicator, View} from 'react-native';
    ...
    const toggleSwitch = () => {
     ...
     const createGroup = async () => {
        ...
        await fetch(
            ...
            `${global.x}/addGroup?admin_id=${props.reducer.userInfo[0]}&playground_id=${props.reducer.playgroundId}&group_name=${name.trim()}&status=${status}&member=${props.reducer.userInfo[0]}&waiting=${props.reducer.userInfo[0]}&invited=${props.reducer.userInfo[0]}`,
            { method: "POST" }
          ).catch((error) => {
            console.log(error)
          })
...
<Body>
{status === 'public'?<AntDesign name="check" size={24} color="green" />:null}
</Body>
...
<Body>
{status === 'private'?<AntDesign name="check" size={24} color="green" />:null}
</Body>
...
<Body>
{status === 'hidden'?<AntDesign name="check" size={24} color="green" />:null}
</Body>
...
<Button style ={{margin: 10,marginTop:50, marginBottom:40}}
...
const mapStateToProps = (state) => {
  ...
  const mapDispachToProps = dispatch => {
    return {
      createGroupTrigger: (x) => dispatch({ type: "CREATE_GROUP_TRIGGER", value: x})
