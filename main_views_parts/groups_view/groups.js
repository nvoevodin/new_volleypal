...
import {deleteGroup, requested,leaveGroup,joinHiddenGroup,requestToJoin,joinGroup} from './functions/groupFunctions'
...
<List>
          ...
          <View key = {index}>
            {(object["status"] == 'hidden' && object.invited.some(i => i === props.reducer.userInfo[0])) || (object["status"] == 'hidden' && object.members.some(i => i === props.reducer.userInfo[0])) || object["status"] != 'hidden'?
         
          <ListItem>

  <Left>
  <TouchableOpacity style = {{flexDirection:'row'}} onPress = {() => {{object["status"] === 'public' || object['admin_id'] === props.reducer.userInfo[0] || object.members.some(i => i === props.reducer.userInfo[0])?selectGroup(object["group_name"], object["group_id"], object['admin_id']):alert("Private group.")}}}>
...
<TouchableOpacity onPress={() => {deleteGroupFunc(object["group_id"], object["members"].length)}}>
              <Text style = {{fontSize:18, fontWeight:'bold', color:'red'}}>Delete</Text>
            </TouchableOpacity>:
object.waiting.some(i => i === props.reducer.userInfo[0])?  
  ...
  <TouchableOpacity onPress={() => {leaveGroup(props.reducer.userInfo[0], object["group_id"])}}>
              <Text style = {{fontSize:18, fontWeight:'bold', color:'red'}}>Leave</Text>
            </TouchableOpacity>:
                        object.invited.some(i => i === props.reducer.userInfo[0])?  
                        <TouchableOpacity onPress={() => {joinHiddenGroup(props.reducer.userInfo[0], object["group_id"])}}>
...
object["status"] == 'public'?
  <TouchableOpacity onPress={() => {joinGroup(props.reducer.userInfo[0], object["group_id"])}}>
              <Text style = {{fontSize:18, fontWeight:'bold', color:'green'}}>Join</Text>
            </TouchableOpacity>:<TouchableOpacity onPress={() => {requestToJoin(props.reducer.userInfo[0], object["group_id"])}}>
...
const styles = StyleSheet.create({
  ...
  const mapDispachToProps = dispatch => {
    return {
        ...
        setGroupInfo: (title, groupId, adminId) => dispatch({ type: "SET_GROUP_INFO", value: [title, groupId, adminId]})
