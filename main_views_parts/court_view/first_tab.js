...
const MainList = (props) => {
  ...
  useEffect(() => {
    (async () => {
      var checkedStatus = await checkedInCheck(
        props.reducer.userInfo[0],
        props.reducer.playgroundId
      );
      setCheckedIn(checkedStatus);

      var preCheckedStatus = await preCheckedInCheck(
        props.reducer.userInfo[0],
        props.reducer.playgroundId
      );
...
const mapDispachToProps = (dispatch) => {
  return {
    toggle_timer_modal: (x) =>
      dispatch({ type: "TOGGLE_TIMER_MODAL", value: x }),
    cancelPreCheck: () => dispatch({ type: "TOGGLE_PRECHECK", value: false }),
  };
};

export default connect(mapStateToProps, mapDispachToProps)(MainList);
