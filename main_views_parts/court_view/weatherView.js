...
const WeatherView = (props) => {
    ...
    useEffect(() => {
        ...
        .then( async (res)  => {
            ...
            {weather !== null?(new Date()).getTime() - moment(res.data[0]["weather_datetime"]).valueOf() > 3560000?updateWeather():null:updateWeather()}
    ...
    const updateWeather = async () =>{
        ...
        await fetch(`http://api.openweathermap.org/data/2.5/weather?lat=${props.reducer.playgroundLat}&lon=${props.reducer.playgroundLon}&units=imperial&appid=e9eca7a76db49a5c816dde3e666730db`)
        .then(res => res.json())
        .then(res => {
...
setLastUpdated(moment())
    ...
    fetch(
        ...
        `${global.x}/addTemperature?playground_id=${props.reducer.playgroundId}&weather=${[res["main"]['temp'],res["weather"][0]["main"],res["wind"]["speed"]]}&weather_datetime=${moment().utc().format("YYYY-MM-DD HH:mm:ss").substr(0, 18) + "0"}`,
        { method: "POST" }
      ).catch((error) => {
        console.log(error)
      })
...
<View>
          <Separator bordered>
              ...
              <Text>WEATHER</Text>
              <TouchableOpacity onPress = {()=>{(new Date()).getTime() - moment(lastUpdated).valueOf() > 3560000?updateWeather():alert('too soon')}}>
          ...
          <ListItem noIndent style={{ backgroundColor: overall === 'Acceptable'?'rgba(8, 171, 253, 0.47)':overall === 'Good'?'rgba(85, 253, 8, 0.47)':overall === 'Bad'?"rgba(253, 8, 8, 0.47)":'rgba(2, 197, 38, 0.6)'}}>
            <Left><Text>Overall</Text></Left>
            <Text>{overall}</Text>
          </ListItem>
          <ListItem noIndent>
          <Left><Text>Temperature</Text></Left>
            <Text>{temperature}</Text>
          </ListItem>
          <ListItem noIndent>
          <Left><Text>Conditions</Text></Left>
            <Text>{conditions}</Text>
          </ListItem>
          <ListItem noIndent>
          <Left><Text>Wind</Text></Left>
            <Text>{wind}</Text>
          </ListItem>
