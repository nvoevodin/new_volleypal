...
console.log('storage')
          ...
          if(res === null){
            ...
            .then(async(res) => {
              ...
              try{
                ...
                props.storeUserId(res.data[0]["uid"],res.data[0]["first_name"], res.data[0]["last_name"], res.data[0]["email"])
              } catch (e){
  ...
  .then(() => {firebase.auth().currentUser.emailVerified?props.signedIn(true):alert('Verify your email please!')})
...
rounded
danger
onPress={() => {firebase.auth().currentUser === null?setResendEmailModalVisible(!resendEmailModalVisible):resendEmailIfLogged()}}
>
...
{forgotPasswordModalVisible && ( 
                  <ForgotPassword 
                    ...
                    toggleModal = {() => setForgotPasswordModalVisible(!forgotPasswordModalVisible)}
         
                  />)}
            {resendEmailModalVisible && ( 
<ResendEmail 
modalVisible = {resendEmailModalVisible}
toggleModal = {()=>setResendEmailModalVisible(!resendEmailModalVisible)}
