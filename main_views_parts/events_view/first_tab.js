...
const MainEvent = (props) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (receivedMembers) {
        ...
        (await receivedMembers.some(
          (i) => i["email"] === props.reducer.userInfo[3]
        ))
          ? setJoined(true)
          : setJoined(false);
      }
  ...
  const InviteInfo = () => {
    Alert.alert(
      `Invite a member.`,
      `You can invite other participants here. The invitation will send a message through a social media that you choose. The message will ask to download the app to join the event.`,
      [{ text: "Got it", onPress: () => {} }],
      { cancelable: false }
    );
  };
...
export default connect(mapStateToProps, mapDispachToProps)(MainEvent);
