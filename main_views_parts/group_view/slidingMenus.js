              ...
              <ListItem key={index}>
                ...
                <Body>
                  {players[1].map((x, j) =>
                    x["user_id"] === object["uid"] ? (
                      <Text style={{ fontSize: 11 }} key={j}>
                        {moment(x["pre_checkin_datetime"]).format("LT")}
                      </Text>
                    ) : null
                  )}
                </Body>
                ...
                <Right>
                  {players[0].map((x, j) =>
                    x["user_id"] === object["uid"] ? (
                      <Text style={{ fontSize: 11 }} key={j}>
                        {moment(x["checkin_datetime"]).format("LT")}
                      </Text>
                    ) : null
                  )}
                </Right>
            ...
            <List>
              ...
              {invitePrompt && (
                <ListItem>
                  ...
                  <TouchableOpacity
                    onPress={
                      (() => sendInvite(props.groupId, object["email"]),
                      (closePrompt = setInvitePrompt(false)))
                    }
                  >
                ...
                <ListItem key={index}>
                  ...
                  <Right>
                    {object["email"] === props.adminId ? (
                      ...
                      <TouchableOpacity
                        onPress={
                          (() => addToGroup(props.groupId, object["email"]),
                  ...
                  <Right>
                    {object["email"] === props.adminId ? (
                      ...
                      <TouchableOpacity
                        onPress={
                          (() =>
                            removeFromGroup(props.groupId, object["email"]),
                          (changeState = setChangeState(!changeState)))
                        }
                      >
...
export default connect(mapStateToProps, mapDispachToProps)(GroupTabs);
